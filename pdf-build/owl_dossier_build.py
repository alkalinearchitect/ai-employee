#!/usr/bin/env python3
# owl_dossier_build.py — generate Owl dossier PDF
import sys
sys.path.insert(0, "/root/ai-employee/pdf-build")
from owl_dossier_content import PAGES, COVER, SITE, BOOK
...
def bg_shape(c, d):
    ...
    c.setFillColor(ACC); c.setFillAlpha(0.10); c.circle(A4[0]*0.82, A4[1]*0.92, 230, fill=1, stroke=0)
    c.setFillAlpha(0.05); c.circle(A4[0]*0.2, A4[1]*0.1, 180, fill=1, stroke=0); c.setFillAlpha(1)
    c.setStrokeColor(ACC); c.setStrokeAlpha(0.35); c.setLineWidth(0.6)
    c.line(18*mm, 14*mm, A4[0]-18*mm, 14*mm)
    c.setFillColor(SOFT); c.setFont("Helvetica", 8)
    c.drawString(18*mm, 9*mm, "OWL DOSSIER · Human Architect")
...
def cover_bg(c, d):
    c.saveState()
    c.setFillColor(BG); c.rect(0, 0, A4[0], A4[1], fill=1, stroke=0)
    c.setFillColor(ACC); c.setFillAlpha(0.14); c.circle(A4[0]*0.5, A4[1]*0.62, 300, fill=1, stroke=0)
    c.setFillAlpha(0.06); c.circle(A4[0]*0.5, A4[1]*0.62, 420, fill=1, stroke=0); c.setFillAlpha(1)
    ...
    for pg in PAGES:
        ...
        for kind, txt in pg["blocks"]:
            if kind == "p":
                ...
                tb = Table(rows, colWidths=[fw*0.30, fw*0.32, fw*0.38])
...
if __name__ == "__main__":
    build("/root/ai-employee/pdf-build/owl-dossier.pdf")
