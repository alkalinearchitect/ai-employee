...
from owl_unified_content_spec import COVER_DOSSIER, DOSSIER_PAGES, URLS
...
def bg_shape(c, d):
    ...
    c.drawString(LM, BM-6*mm, 'OWL DOSSIER · Human Architect')
...
def build(path):
    ...
    st.append(Paragraph(COVER_DOSSIER["eyebrow"], ss["eyebrow"]))
    st.append(Paragraph(COVER_DOSSIER["title"], ss["title"]))
    st.append(HRFlowable(width="35%", thickness=1, color=ACC, spaceAfter=10, spaceBefore=0, hAlign="LEFT"))
    st.append(Paragraph(COVER_DOSSIER["sub"], ss["sub"]))
    st.append(Paragraph(COVER_DOSSIER["tag"], ss["label"]))
    st.append(Spacer(1, 8*mm))
    st.append(Paragraph(COVER_DOSSIER["foot"], ss["foot"]))
    ...
    for pg in DOSSIER_PAGES:
        ...
        for blk in pg["blocks"]:
            ...
            if kind == "p":
                ...
                col_w = [fw * 0.30, fw * 0.32, fw * 0.38]
                if len(rows[0]) == 5:
                    col_w = [fw * 0.18, fw * 0.28, fw * 0.28, fw * 0.16, fw * 0.10]
