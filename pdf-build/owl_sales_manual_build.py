...
from owl_unified_content_spec import COVER_SALES, SALES_PAGES, URLS, OBJECTIONS_TABLE
...
def bg_shape(c, d):
    ...
    c.drawString(LM, BM-6*mm, 'OWL CLOSING MANUAL · Human Architect')
...
def build(path):
    ...
    st.append(Paragraph(COVER_SALES["eyebrow"], ss["eyebrow"]))
    st.append(Paragraph(COVER_SALES["title"], ss["title"]))
    st.append(HRFlowable(width="35%", thickness=1, color=ACC, spaceAfter=10, spaceBefore=0, hAlign="LEFT"))
    st.append(Paragraph(COVER_SALES["sub"], ss["sub"]))
    st.append(Paragraph(COVER_SALES["tag"], ss["label"]))
    st.append(Spacer(1, 8*mm))
    st.append(Paragraph(COVER_SALES["foot"], ss["foot"]))
    st.append(Paragraph("Live: " + URLS["SITE"] + "<br/>Call: " + URLS["BOOK"], ss["foot"]))
    st.append(PageBreak())

    for pg in SALES_PAGES:
        ...
        for blk in pg["blocks"]:
            ...
            if kind == "p":
                ...
                col_w = [fw * 0.28, fw * 0.24, fw * 0.38, fw * 0.10] if len(rows[0]) == 4 else [fw * 0.28, fw * 0.36, fw * 0.36]
...
if __name__ == "__main__":
    build("/root/ai-employee/pdf-build/owl_sales_manual.pdf")
