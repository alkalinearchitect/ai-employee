...
function mountScrollWorld(container, config) {
  ...
  function read() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      else if (i === N - 1) cop = (before || after) ? 0 : smooth(1 - Math.abs(pr - 0.5) / 0.5);  // fades out like other sections, no permanent overlay
...
function injectCSS() {
  ...
  .sw-copylayer::before{content:"";position:absolute;inset:0;width:min(66vw,880px);background:linear-gradient(90deg,var(--sw-bg) 0%,color-mix(in srgb,var(--sw-bg) 92%,transparent) 30%,color-mix(in srgb,var(--sw-bg) 64%,transparent) 58%,color-mix(in srgb,var(--sw-bg) 30%,transparent) 80%,transparent 100%);}
  ...
  .sw-copy__title{font-family:var(--sw-font-display);font-weight:700;color:var(--sw-ink);font-size:clamp(2rem,4.4vw,3.5rem);line-height:1.03;margin:12px 0 0;letter-spacing:-.01em;text-shadow:0 2px 24px color-mix(in srgb,var(--sw-bg) 88%,transparent),0 1px 3px rgba(0,0,0,.65);}
  .sw-copy__body{margin-top:18px;font-size:clamp(1rem,1.25vw,1.14rem);line-height:1.6;color:var(--sw-ink);max-width:40ch;text-shadow:0 1px 14px color-mix(in srgb,var(--sw-bg) 95%,transparent),0 1px 2px rgba(0,0,0,.6);}
  ...
  @media (max-width:860px){
    .sw-nav{display:none;}
    .sw-copylayer::before{width:100%;height:74%;top:auto;bottom:0;background:linear-gradient(0deg,var(--sw-bg) 10%,color-mix(in srgb,var(--sw-bg) 86%,transparent) 44%,color-mix(in srgb,var(--sw-bg) 40%,transparent) 72%,transparent 100%);}
